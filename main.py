...
def MontyHallSimulation(N):
    ...
    for _ in range(0, N):
        ...
        HostOpens = random.choice(list({'Door 1', 'Door 2', 'Door 3'} - {FirstSelection, WinningDoor}))

        # 4) The other door is not the participant's selected door and not the opened door.
        OtherDoor = random.choice(list({'Door 1', 'Door 2', 'Door 3'} - {FirstSelection, HostOpens}))
